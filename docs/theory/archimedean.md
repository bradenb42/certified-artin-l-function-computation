...
**Procedure.** Count $r$ (Sturm on $f$). If $r=n$, $c=1$. Otherwise locate the block $\mathcal{B}*\lambda$, $\lambda=1^r2^{(n-r)/2}$, from the table (classes of involutions of that cycle type, by the power maps of* [*the character table*](https://claude.ai/chat/character-table.md)*); if $\mathcal{B}*\lambda$ is a single class, done; else apply the separating family $\mathcal{H}*\lambda$ of* [*separating subgroups*](https://claude.ai/chat/separating-subgroups.md) *with $\mathrm{fix}*{G/H}(c)$ = real-root count of $R\_{G,H,F}$ (Sturm on the exact resolvents already computed at $p$), and read the class by [rational classes](https://claude.ai/chat/rational-classes.md) Theorem 4.1(1). No archimedean numerics of the roots themselves and no matching at the infinite place are needed. As a check, $c$ must satisfy $\chi(c)\in\mathbb{Z}$ with $\chi(c)\equiv\chi(1)\pmod 2$ for every $\chi$ (§2).
...
and $\Lambda(s,\chi)=\mathfrak{f}(\chi)^{s/2}\gamma\_\chi(s)L(s,\chi)$ is the completed $L$-function of [the character table](https://claude.ai/chat/character-table.md) Lemma 5.3.
...
which are the Gamma factor and archimedean sign of a holomorphic newform of weight one, and $\lambda\_\chi=\det\rho\_\chi$ is an odd Dirichlet character (the nebentypus), $\lambda\_\chi(c)=(-1)^{b\_\chi}=-1$, in agreement with [the global conductor](https://claude.ai/chat/global-conductor.md) Prop. 2.1 evaluated at $c$.
