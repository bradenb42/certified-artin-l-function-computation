...
**Cycle-type blocks.** For a class $C$ let $\mathrm{ct}(C)$ be the cycle type of its elements on ${1,\dots,n}$, a partition $\lambda\vdash n$. The *block* of $\lambda$ is $\mathcal{B}*\lambda={C:\mathrm{ct}(C)=\lambda}$. By* [*rational classes*](https://claude.ai/chat/rational-classes.md)*, Prop. 1.2(3), the factorization of $f$ modulo $\ell$ locates $\mathrm{Frob}*\ell$ in a block and no further.
...
*Proof.* $\sigma$ and $\sigma^k$ with $\gcd(k,o(\sigma))=1$ generate the same cyclic group and so have the same cycle type; hence $\mathrm{ct}$ is constant on rational classes. The order of a permutation is the lcm of its cycle lengths, so two classes in $\mathcal{B}\_\lambda$ have the same order $\mathrm{lcm}(\lambda)$; their cyclic subgroups are non-conjugate (distinct rational classes, [rational classes](https://claude.ai/chat/rational-classes.md) Lemma 2.1(a)) and of equal order, and a subgroup of a cyclic group of the same order is the whole group. $\square$

**Sub-blocks.** Write $\mathcal{B}*\lambda=\rho*{\lambda,1}\sqcup\dots\sqcup\rho\_{\lambda,r\_\lambda}$ as a union of rational classes. By [rational classes](https://claude.ai/chat/rational-classes.md) Theorem 4.1, coset actions can separate the $\rho\_{\lambda,j}$ from one another and cannot split any $\rho\_{\lambda,j}$ into its conjugacy classes. So the sub-blocks that coset actions must separate are exactly the rational classes, and the separation task is:

$$U=\Big{{\rho,\rho'}:\ \rho\ne\rho'\text{ rational classes in the same block}\Big},\qquad u:=|U|=\sum\_\lambda\binom{r\_\lambda}{2}.$$

Blocks with $r\_\lambda=1$ contribute nothing; if all $r\_\lambda=1$ the degree-$n$ action already determines the rational class ([rational classes](https://claude.ai/chat/rational-classes.md) Cor. 4.3). The partition is computed by hashing the class representatives on cycle type and then applying the union to find of [rational classes](https://claude.ai/chat/rational-classes.md) §2 inside each block.

**What a subgroup separates.** For $H\in\mathcal{S}$ define the *$H$-signature* of a rational class $\rho$ with generator $\sigma$ as the cycle type of $\sigma$ on $G/H$, i.e. the function $d\mapsto\pi\_H(\sigma^d)$ for $d\mid o(\sigma)$, computed from [rational classes](https://claude.ai/chat/rational-classes.md) Lemma 1.1 and the power maps ($|H|$ class identifications per $H$). $H$ *covers* ${\rho,\rho'}\in U$ if the signatures differ; let $S\_H\subseteq U$ be the set of covered pairs. Since the full factorization type of a resolvent over $\mathbb{Q}\_\ell$ is available at the same cost as the count of degree-one factors ([rational classes](https://claude.ai/chat/rational-classes.md) Prop. 1.2), the signature is the right notion; using only $\pi\_H(\sigma)$ gives a sub-instance with smaller $S\_H$ and every statement below still holds.
...
**Construction.** Fix once and for all a base $t=(i\_1,\dots,i\_k)$ of $G$: a tuple of points whose pointwise stabilizer $G\_t$ is trivial (from the BSGS of [the character table](https://claude.ai/chat/character-table.md); an irredundant base has $k\le\log\_2|G|$). Put $m\_t=\prod\_{j=1}^kx\_{i\_j}^{,j}$ and, for $H\le G$,
...
**Proposition 3.1.** (a) $\mathrm{Stab}\_G(F\_H)=H$. (b) $\deg F\_H=\binom{k+1}{2}$, $|F\_H|*1=|H|$, and $F\_H$ has $|H|$ distinct monomials. (c) $F\_H$ has integer coefficients, so $B*{F\_H}=|H|R\_0^{\binom{k+1}2}$ in [the precision policy](https://claude.ai/chat/precision-policy.md)'s notation.
...
**Resolvent.** $R\_{G,H,F}(x)=\prod\_{\tau\in G/H}(x-F(\tau\alpha))$, of degree $m=[G\:H]$, with $\mathbb{Z}$-coefficients bounded by [the precision policy](https://claude.ai/chat/precision-policy.md) Lemma 1.2; it is computed exactly from the $p$-adic roots at the precision of [the precision policy](https://claude.ai/chat/precision-policy.md) §5 (test T1).

**Squarefreeness.** The $m$ polynomials $\tau F$, $\tau\in G/H$, are pairwise distinct by Proposition 3.1(a), but their values at $\alpha$ may coincide. Let $P\_F=\prod\_{{\tau H\ne\tau'H}}(\tau F-\tau'F)\in\mathbb{Z}[x\_1,\dots,x\_n]$, nonzero of degree $\le\binom m2\deg F$; $R\_{G,H,F}$ is squarefree iff $P\_F(\alpha)\ne0$, decided exactly by $\mathrm{disc}(R\_{G,H,F})\ne0$ ([the precision policy](https://claude.ai/chat/precision-policy.md), T1/T3).
...
After the repair, $F$ is replaced by $F\circ T$ in [the precision policy](https://claude.ai/chat/precision-policy.md)'s list $\mathcal{F}$: $\deg(F\circ T)=\deg F\cdot\deg T$ and $|F\circ T|\_1\le|F|\_1|T|\_1^{\deg F}$. Trying $T$ of small degree first keeps these small; Lemma 3.2 is the guarantee that a search over degree $\<n$ terminates.
...
3. For $\ell\mid\mathrm{disc}(R)$ with $\ell\nmid\mathrm{disc}f$, the same quantities are the number of $\mathbb{Q}*\ell$-roots and the $\mathbb{Q}*\ell$-factorization type of $R$, computed at precision $k\_\ell$ ([the precision policy](https://claude.ai/chat/precision-policy.md), T5), since $(F,m)\in\mathcal{F}$.

*Proof.* 1. $G$ permutes the roots $F(\tau\alpha)$ transitively (transitively on $G/H$) and they are distinct, so $R$ is irreducible; $\mathrm{Stab}*G(\beta)={g\:F(g\alpha)=F(\alpha)}=H$ by distinctness, so $\mathbb{Q}(\beta)=N^H$; the splitting field is the Galois closure, fixed by the largest normal subgroup of $G$ inside $H$. 2. $\ell\nmid\mathrm{disc}(R)=\mathrm{disc}(\mathbb{Z}[\beta])$ implies $\mathbb{Z}[\beta]$ is $\ell$-maximal and $\ell\nmid\mathrm{disc},\mathcal{O}*{N^H}$, so $\ell$ is unramified in $N^H$ ([the ramified primes](https://claude.ai/chat/ramified-primes.md) Lemma 1.2), hence in its Galois closure $N^{\mathrm{core}*G(H)}$ (*[*the ramified primes*](https://claude.ai/chat/ramified-primes.md) *Theorem 1.1), i.e. $I*\ell\le\mathrm{core}*G(H)$. Kummer to Dedekind for the $\ell$-maximal order $\mathbb{Z}[\beta]$: the factorization of $R$ modulo $\ell$ mirrors the splitting of $\ell$ in $N^H$, whose residue degrees are the orbit lengths of the Frobenius of $N^{\mathrm{core}}$ on the roots, i.e. of $\phi$ (any element of $\phi I*\ell$) on $G/H$; the orbits of length one are the fixed cosets. 3. [the precision policy](https://claude.ai/chat/precision-policy.md) Prop. 3.4(4) with $h=R$, as $\ell^{k\_\ell}>M\_F\ge\ell^{v\_\ell(\mathrm{disc}R)}$. $\square$
...
*Proof.* A distinct primes' product dividing a nonzero integer $D$ is at most $|D|$, so their number is at most $\log\_2|D|$; the bound on $|\mathrm{disc}R|$ is [the precision policy](https://claude.ai/chat/precision-policy.md) Lemma 1.2 and $B\_F=|F|\_1R\_0^{\deg F}$. $\square$
...
**Summary of the family attached to $G$.** Before the unramified-prime loop: blocks and sub-blocks (§1), signatures for $H\in\mathcal{S}$, greedy cover $\mathcal{H}$ (§2) with weight bounded by Theorem 2.2, invariants $F\_H$ (§3) repaired by Lemma 3.2 until every $R\_{G,H,F}$ is squarefree, the pairs $(F\_H,[G\:H])$ appended to $\mathcal{F}$ so that [the precision policy](https://claude.ai/chat/precision-policy.md)'s $\mathbf{M}$ covers them, the exact resolvents and their discriminants, and the excluded sets $E\_H$. At each $\ell\le X$: factor $f$ modulo $\ell$ (block); factor each $R\_{G,H,F}$, $H\in\mathcal{H}$, modulo $\ell$ or $\ell$-adically according to $\ell\notin E\_H$ or $\ell\in E\_H$ (rational class, by [rational classes](https://claude.ai/chat/rational-classes.md) Theorem 4.1); then [the precision policy](https://claude.ai/chat/precision-policy.md) §4 for the conjugacy class when the rational class is not a single class.