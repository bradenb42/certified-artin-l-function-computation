...
1. From [local Galois groups](local-galois-groups.md): $D$, $L$ in the tower, $\nu$.
2. From [the filtration](ramification-filtration.md) §1: $I$, $P$, $\Phi$.
3. Either [the filtration](ramification-filtration.md) §2 ($i\_L$ from $\varpi\_L$), or, when $L$ is avoided: the polygons of the factor fields and pairwise composita ($\mathcal{H}$), the candidate set $\mathcal{C}(\mathcal{H})$ of Theorem 1.3 filtered by Check 1, and, if $|\mathcal{C}(\mathcal{H})|>1$, the resolvent $R\_S$ of §2 for a minimal-index $S\in\mathcal{S}$: its Newton polygon fixes the orders, its labeled roots (in $L^{\mathrm{core}\_D(S)}$) fix the subgroups.
4. Check 1 and Check 2 on the result; then the Herbrand conversion of [the filtration](ramification-filtration.md) §4 and the transport to $G$ of [matching](matching.md) to [well-definedness](well-definedness.md).
